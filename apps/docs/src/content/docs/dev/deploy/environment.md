---
title: "環境變數"
description: "所有環境變數在啟動時由 @huan/config 以 zod 驗證。缺少或格式錯誤時 Server 與 Worker 會拒絕啟動，並明確指出是哪一個變數有問題——不會帶著錯誤的設定跑起來，然後在某個奇怪的地方失敗。"
---
...
Server 簽出來的網址是給**瀏覽器和裝置**用的。Server 在 Docker 網路裡透過 `http://rustfs:9000` 連到儲存，但那個主機名稱在容器外面不存在。兩者不同時就必須分開設定，否則簽出來的網址在外部連不上。
