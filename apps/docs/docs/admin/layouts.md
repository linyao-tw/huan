...
理由見 [ADR-0004](/architecture/adr/0004-recursive-split-layout)。
...
::: tip 拖曳不是唯一的方式每一個拖曳操作都有鍵盤或表單的替代方式。比例可以用方向鍵微調，也可以直接輸入數字。:::
...
::: warning 很多網站不能被嵌入外部網站可以用 `X-Frame-Options` 或 `Content-Security-Policy: frame-ancestors` 明確禁止被嵌入。遇到這種網站，區塊會顯示空白。
