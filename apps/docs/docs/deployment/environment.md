...
調長只是多花儲存空間；調短則會更快遇到「需要重新上傳」的狀態。詳見 [ADR-0003](/architecture/adr/0003-temporary-object-storage)。
...
| 變數               | 說明                                         |
| ------------------ | -------------------------------------------- |
| `DOCS_BASE`        | GitHub Pages 的子路徑，例如 `/huan/`         |
| `DOCS_SITE_ORIGIN` | 站台來源，例如 `https://linyao-tw.github.io` |

這兩個值由 GitHub Actions 自動帶入，不需要手動設定。
