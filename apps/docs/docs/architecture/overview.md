...
這樣做的理由見 [ADR-0001](/architecture/adr/0001-websocket-plus-rest)。
...
工作佇列就是 PostgreSQL 的一張表，用 `SELECT ... FOR UPDATE SKIP LOCKED` 取件。這個 MVP 不需要 Redis（[ADR-0007](/architecture/adr/0007-postgres-job-queue)）。
