...
async function loadSchedule(db: Database, scheduleId: string): Promise<Schedule | null> {
	const [row] = await db.select({ schedule: schedules, layoutName: layouts.name }).from(schedules).innerJoin(layouts, eq(layouts.id, schedules.layoutId)).where(eq(schedules.id, scheduleId)).limit(1);
	if (!row) return null;
...
async function assertReferencesExist(db: Database, layoutId: string, deviceIds: readonly string[]): Promise<void> {
	const [layout] = await db.select({ id: layouts.id }).from(layouts).where(eq(layouts.id, layoutId)).limit(1);
...
export const scheduleRoutes: FastifyPluginAsyncZod = async app => {
	...
	app.get(
		"/schedules",
		{ preHandler: requireSession, schema: { tags: ["schedules"], summary: "列出排程", querystring: PaginationQuerySchema, response: { 200: ScheduleListResponseSchema } } },
		async request => {
			...
			const rows = await db
				...
				.limit(limit)
				.offset(offset);
			const [total] = await db.select({ value: count() }).from(schedules);
	...
	app.post(
		"/schedules",
		{ preHandler: requireSession, schema: { tags: ["schedules"], summary: "建立排程", body: CreateScheduleRequestSchema, response: { 201: ScheduleSchema } } },
		async (request, reply) => {
			const actor = sessionOf(request);
			const body = request.body;
			await assertReferencesExist(db, body.layoutId, body.deviceIds);
			...
			if (body.deviceIds.length > 0) {
				await db.insert(scheduleDevices).values(body.deviceIds.map(deviceId => ({ scheduleId: created.id, deviceId })));
			}
			...
			const schedule = await loadSchedule(db, created.id);
	...
	app.patch(
		...
		async request => {
			const actor = sessionOf(request);
			const body = request.body;
			const [existing] = await db.select().from(schedules).where(eq(schedules.id, request.params.id)).limit(1);
			if (!existing) throw notFound("找不到這個排程");
			await assertReferencesExist(db, body.layoutId, body.deviceIds);
			...
			if (body.deviceIds.length > 0) {
				await db.insert(scheduleDevices).values(body.deviceIds.map(deviceId => ({ scheduleId: existing.id, deviceId })));
			}
			...
			const schedule = await loadSchedule(db, existing.id);
			if (!schedule) throw notFound("找不到這個排程");
			return schedule;
		}
	);

	app.delete("/schedules/:id", { preHandler: requireSession, schema: { tags: ["schedules"], summary: "刪除排程", params: z.object({ id: IdSchema }), response: { 200: OkSchema } } }, async request => {
		const actor = sessionOf(request);
		const [existing] = await db.select().from(schedules).where(eq(schedules.id, request.params.id)).limit(1);
		if (!existing) throw notFound("找不到這個排程");
